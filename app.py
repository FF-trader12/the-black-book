...
VERSION = "THE BLACK BOOK v4.0.1"
LATEST_CHANGES = [
    "General chat is permanently ignored by the bot.",
    "Football SVR, Football Accas, Football Results and Members Chat routing is locked to topics.",
    "Scheduler runs 07:30 Morning Briefing, 12:00 Match Day Update, 20:00 Tomorrow Preview + Accas, and 5-minute saved-fixture alerts.",
    "Final Call and Bets Closed alerts read saved fixtures only, protecting free Odds API credits.",
    "Local JSON storage added for fixtures, members, stats, alerts, version and cache.",
    "Duplicate protection added for scheduled posts, welcomes and version updates.",
...
# v4.0.1 Foundation config
MEMBERS_CHAT_TOPIC_ID = int(os.environ.get("MEMBERS_CHAT_TOPIC_ID", "421") or 421)
DATA_DIR = Path(os.environ.get("BLACK_BOOK_DATA_DIR", "/tmp/the_black_book"))
FIXTURE_STORE_FILE = os.environ.get("FIXTURE_STORE_FILE", str(DATA_DIR / "fixtures.json"))
MEMBERS_FILE = os.environ.get("MEMBERS_FILE", str(DATA_DIR / "members.json"))
STATS_FILE = os.environ.get("STATS_FILE", str(DATA_DIR / "stats.json"))
ALERTS_FILE = os.environ.get("ALERTS_FILE", str(DATA_DIR / "alerts.json"))
VERSION_FILE = os.environ.get("VERSION_FILE", str(DATA_DIR / "version.json"))
CACHE_FILE = os.environ.get("CACHE_FILE", str(DATA_DIR / "cache.json"))
SCHEDULER_STATE_FILE = os.environ.get("SCHEDULER_STATE_FILE", str(DATA_DIR / "scheduler.json"))
RELEASE_NOTES_FILE = VERSION_FILE
...
class BlockedTelegramResponse:
    status_code = 200
    text = "general_chat_blocked"

    def json(self):
        return {"ok": True, "blocked": "general_chat_ignored"}


def is_general_chat_target(chat_id, thread_id):
    # In a Telegram forum supergroup, messages without a topic/thread ID are General.
    # The Black Book v4.0.1 never posts there.
    return str(chat_id) == str(FOOTBALL_CHAT_ID) and thread_id is None
def send_telegram_message(chat_id, text: str, thread_id=None):
    if is_general_chat_target(chat_id, thread_id):
        return BlockedTelegramResponse()
...
def football_cache_key(target_date=None, sport_keys=None):
    date_label = scan_date_label(target_date or now_utc().date())
    keys = ",".join(sport_keys or FOOTBALL_SPORT_KEYS)
    return f"{date_label}|{keys}"


def save_football_cache(target_date, sport_keys, events, errors=None):
    cache = load_cache_state()
    key = football_cache_key(target_date, sport_keys)
    cache.setdefault("football", {})[key] = {
        "saved_at": now_utc().isoformat(),
        "target_date": scan_date_label(target_date or now_utc().date()),
        "sport_keys": sport_keys or FOOTBALL_SPORT_KEYS,
        "events": events or [],
        "errors": errors or [],
    }
...
def read_football_cache(target_date=None, sport_keys=None, max_age_hours=18):
    cache = load_cache_state()
    key = football_cache_key(target_date, sport_keys)
    record = cache.get("football", {}).get(key)
    if not record:
    ...
    try:
        ...
        age_hours = (now_utc() - saved_at.astimezone(timezone.utc)).total_seconds() / 3600
        if age_hours > max_age_hours:
            return None, ["cache expired"]
    except Exception:
        return None, ["cache timestamp invalid"]
    return record.get("events", []), record.get("errors", [])


def refresh_football_cache(target_date=None, sport_keys=None):
    events, errors = fetch_football_odds(target_date, sport_keys)
    save_football_cache(target_date, sport_keys or FOOTBALL_SPORT_KEYS, events, errors)
    return events, errors


def cached_or_refresh_football_odds(target_date=None, sport_keys=None):
    events, errors = read_football_cache(target_date, sport_keys)
    if events is not None:
        return events, errors
    return refresh_football_cache(target_date, sport_keys)
...
def build_daily_acca_message(target_date=None, sport_keys=None, league_key=None):
    events, errors = cached_or_refresh_football_odds(target_date, sport_keys)
    rows = []
...
def build_tomorrow_preview_message():
    ...
    events, errors = cached_or_refresh_football_odds(target_date, FOOTBALL_SPORT_KEYS)
    rows = []
...
def load_members_state():
    data = read_json_file(MEMBERS_FILE, {"welcomed": {}})
...
def save_members_state(state):
    write_json_file(MEMBERS_FILE, state)


def load_stats_state():
    data = read_json_file(STATS_FILE, {"results": [], "summary": {}})
    return data if isinstance(data, dict) else {"results": [], "summary": {}}


def save_stats_state(state):
    write_json_file(STATS_FILE, state)


def load_cache_state():
    data = read_json_file(CACHE_FILE, {"football": {}})
    if not isinstance(data, dict):
        data = {"football": {}}
    data.setdefault("football", {})
    return data


def save_cache_state(state):
    write_json_file(CACHE_FILE, state)


def is_admin_user(user_id):
    if not ADMIN_USER_IDS:
        return True
    return str(user_id or "").strip() in ADMIN_USER_IDS


def admin_only_message():
    return (
        "🔒 <b>ADMIN ONLY</b>\n\n"
        "This command is restricted to The Black Book admins."
    )
...
def build_and_store_fixtures(target_date, source="scan"):
    events, errors = refresh_football_cache(target_date, FOOTBALL_SPORT_KEYS)
    rows = []
    for event in events:
        scored = score_football_event(event)
        if not scored:
...
def handle_new_members(update):
    ...
    welcomed = members_state.setdefault("welcomed", {})
    sent = 0
    ...
    for member in new_members:
        ...
        member_id = str(member.get("id") or member.get("username") or "").strip()
        if member_id and welcomed.get(member_id):
            continue
...
def build_storage_status_message():
    store = load_fixture_store()
    members = load_members_state()
    stats = load_stats_state()
    cache = load_cache_state()
    return (
        "💾 <b>THE BLACK BOOK STORAGE</b>\n\n"
        f"fixtures.json: <b>{len(store.get('fixtures', {}))} saved fixtures</b>\n"
        f"members.json: <b>{len(members.get('welcomed', {}))} welcomed members</b>\n"
        f"stats.json: <b>{len(stats.get('results', [])) if isinstance(stats.get('results', []), list) else 0} results</b>\n"
        f"alerts.json: <b>{Path(ALERTS_FILE).name}</b>\n"
        f"version.json: <b>{VERSION}</b>\n"
        f"cache.json: <b>{len(cache.get('football', {}))} cache groups</b>\n\n"
        "📌 <b>Current setup</b>\n"
        f"{LOCAL_DATA_NOTICE}\n\n"
        "Permanent Render disk storage can be added later so these JSON files persist through updates."
    )
...
def build_version_update_message():
    return (
        ...
        f"<b>{VERSION}</b> is now live.\n\n"
        "✅ <b>Latest Changes</b>\n"
        f"{latest_changes_text()}\n\n"
        "📌 <b>Data note</b>\n"
        f"{LOCAL_DATA_NOTICE}\n\n"
        "📚 The Black Book\n"
        "Find The Edge."
...
def telegram_webhook():
    try:
        ...
        # General chat is permanently closed in v4.0.1.
        # In Telegram forum groups, General messages have no message_thread_id.
        if chat_type in ("group", "supergroup") and thread_id is None:
            return jsonify({"ok": True, "ignored": "general_chat_closed"}), 200
        ...
        admin_prefixes = ("/score", "/settings", "/chatid", "/versionupdate", "/storage", "/morning", "/routes")
        if lower_text.startswith(admin_prefixes) and not is_admin_user(from_user_id):
            tg_response = send_telegram_message(chat_id, admin_only_message(), thread_id=thread_id)
            return jsonify({"ok": tg_response.status_code == 200, "telegram_status": tg_response.status_code}), 200
        ...
        if lower_text.startswith("/start"):
            ...
            status = "sent to Members Chat" if result.get("sent") else "already sent"
            tg_response = send_telegram_message(chat_id, f"📢 Version update {status}.", thread_id=thread_id)
